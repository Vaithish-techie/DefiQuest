...
function renderQuizResults(result, userAnswers) {
	...
	let resultsHTML = `<div class="results-page"><h1>Quiz Results: ${
		...
		result.success
			// This message still works for a demo, as a "badge" is added to their profile
			? `<p>You've earned ${result.xp_earned} XP and a new badge has been added to your profile!</p>`
...
async function renderProfile() {
	...
	try {
		...
		// Simplified: Only fetch the main profile data
		const profileResponse = await fetch(`${API_BASE}/profile?address=${wallet}`);
		...
		const profileHTML = `
            ...
            <div class="profile-grid">
                ...
                <!-- This is now the single source of truth for badges -->
                <div class="badges-panel"><h3>🏆 My Badges</h3><div class="badges-grid">${badgesHTML}</div></div>
            </div>`;
